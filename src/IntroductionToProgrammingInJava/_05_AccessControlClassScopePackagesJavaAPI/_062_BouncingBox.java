...
public class _062_BouncingBox {
     ...
     * with startColor.
     */
    public _062_BouncingBox(int startX, int startY, Color startColor) {
        x = startX;
        y = startY;
        color = startColor;
    }
    ...
    public void draw(Graphics surface) {
        ...
        if ((x - SIZE/2 <= 0 && xDirection < 0) ||
                (x + SIZE/2 >= _061_SimpleDraw.WIDTH && xDirection > 0)) {
            xDirection = -xDirection;
        }
        if ((y - SIZE/2 <= 0 && yDirection < 0) ||
                (y + SIZE/2 >= _061_SimpleDraw.HEIGHT && yDirection > 0)) {
